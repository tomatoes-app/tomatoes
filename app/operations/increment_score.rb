class IncrementScore
  ...
  def process
    now = Time.current
    add_score(self.user_id, self.score, DailyScore, to_user_timezone(now.end_of_day))
    add_score(self.user_id, self.score, WeeklyScore, to_user_timezone(now.end_of_week))
    add_score(self.user_id, self.score, MonthlyScore, to_user_timezone(now.end_of_month))
    add_score(self.user_id, self.score, OverallScore)
  end
